...
class Game
{
  public:
    ...
    // Отвечает за запуск/перезапуск партии, поочерёдные ходы игроков/ботов,
    // проверку окончания игры и показ финального экрана с результатом.
    int play()
    {
  ...
  private:
    // Ход бота.
    // На основе текущего состояния доски просчитывает лучший ход с помощью минимакса,
    // делает небольшую задержку (чтобы ход выглядел "человечным") и последовательно
    // выполняет все ходы (в том числе серию взятий).
    ...
    // Ход живого игрока.
    // Обрабатывает клики по клеткам: выбор фигуры, выбор клетки назначения,
    // выполнение хода или серии взятий, а также нажатия кнопок BACK/REPLAY/QUIT.
